...
export async function generateMetadata({ params }: CategoryPageProps): Promise<Metadata> {
  ...
  if (isValidLocale(category)) {
    const loc = getLocale(category)!
    return {
      title: `Trading365 — ${loc.name}`,
      description: `Crypto exchange reviews and comparisons in ${loc.fullName}.`,
      alternates: { canonical: `https://www.trading365.org/${category}` },
    }
  }
...
export default async function CategoryPage({ params }: CategoryPageProps) {
  ...
  if (isValidLocale(category)) {
    ...
    try {
      translations = await getAllTranslationsForLocale(category)
    } catch {}
        ...
        <section className="border-b border-border bg-secondary/30">
          <div className="mx-auto max-w-6xl px-4 py-12">
            <div className="flex items-center gap-3 mb-4">
              ...
              <h1 className="text-3xl font-bold text-foreground">Trading365 — {loc.name}</h1>
            </div>
            <p className="text-muted-foreground max-w-2xl">
              Crypto exchange reviews, comparisons, and exclusive bonus deals — in {loc.fullName}.
            </p>
            <Link href="/" className="mt-4 inline-block text-sm text-primary hover:underline">← English version</Link>
          </div>
        </section>
        <section className="mx-auto max-w-6xl px-4 py-10">
          {translations.length === 0 ? (
            <div className="rounded-xl border border-dashed border-border p-12 text-center">
              <p className="text-muted-foreground">{loc.fullName} content is being prepared. Check back soon.</p>
              <Link href="/" className="mt-4 inline-block text-sm text-primary hover:underline">View English content →</Link>
            </div>
          ) : (
            <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
              {translations.map((t) => (
                <Link
                  key={t.article_slug}
                  href={`/${category}/${(t as any).category_slug}/${t.article_slug}`}
