...
export async function generateStaticParams() {
  // Only generate for locales — slugs are dynamic at request time
  return LOCALE_CODES.map((locale) => ({ locale, category: "reviews", slug: "placeholder" }))
}
...
export default async function LocaleArticlePage({
  ...
  if (!translation) {
    // Article not translated yet — redirect to English version
    return (
      <>
        <SiteHeader />
        ...
        <article className="mx-auto max-w-4xl px-4 py-10">
          ...
          <div className="mt-8 border-t border-border pt-6">
            ...
            <div className="flex flex-wrap gap-2">
              <Link href={`/${params.category}/${params.slug}`} className="text-xs rounded-full border border-border px-3 py-1 text-muted-foreground hover:border-primary hover:text-primary">
                🇬🇧 English
              </Link>
