...
// (every downtrend score>=7 setup, fired or not) badly understates this, so it
  ...
  try {
    const rows = side === "short"
      ? await sql`SELECT COUNT(*) FILTER (WHERE tp_result LIKE 'TP%')::int AS wins,
                  ...
                  // Note: the old hardcoded Jul 2–5 exclusion is gone — those
                  // regime-whipsaw shorts were removed at source by the 4H EMA50
                  // deadband cleanup (scripts/delete-whipsaw-shorts.mjs), so the
                  // headline hit rate now derives purely from the clean data.
      : await sql`SELECT COUNT(*) FILTER (WHERE tp_result LIKE 'TP%')::int AS wins,
                         COUNT(*) FILTER (WHERE tp_result = 'SL')::int  AS sl
    ...
    const aggRows = side === "short"
      ? await sql`
        SELECT
          ...
          COUNT(*) FILTER (
            WHERE s.direction = 'short' AND s.market_condition = 'downtrend' AND s.score >= 7 AND o24.pct_change <= -1.5
          )::int AS tp1_hits,
          COUNT(*) FILTER (
            WHERE s.direction = 'short' AND s.market_condition = 'downtrend' AND s.score >= 7 AND o24.pct_change < 0
          )::int AS dir_hits,
